...
VERSION=
commit=false
url=https://github.com/imharrywu/BitcoinTop
proc=2
mem=2000
lxc=true
...
then
	...
	then
	    ...
	    mv build/out/bitcointop-*.tar.gz build/out/src/bitcointop-*.tar.gz ../bitcoin-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    mv build/out/bitcointop-*-win-unsigned.tar.gz inputs/bitcointop-win-unsigned.tar.gz
	    mv build/out/bitcointop-*.zip build/out/bitcointop-*.exe ../bitcoin-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    mv build/out/bitcointop-*-osx-unsigned.tar.gz inputs/bitcointop-osx-unsigned.tar.gz
	    mv build/out/bitcointop-*.tar.gz build/out/bitcointop-*.dmg ../bitcoin-binaries/${VERSION}
	fi
	popd
